...
class ConceptVectorizer:
    ...
    def load_vectorizer(cls, path_to_load):
        ...
        with open(conf_file, 'r') as f:
            config = json.load(f)
        relocate_on_cpu = False
        if not torch.cuda.is_available() and config['use_cuda']:
            warnings.warn("use_cuda is set to true in ConceptVectorizer config, but there is no cuda device. use_cuda will be automatically set to False", UserWarning)
            config['use_cuda'] = False
            relocate_on_cpu = True
        CV = cls(**config)
        if CV.config['path_to_thesaurus_embeddings']:
            if relocate_on_cpu:
                CV.thesaurus_embeddings = torch.load(CV.config['path_to_thesaurus_embeddings'], map_location=torch.device('cpu'))
            else:
