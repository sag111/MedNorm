...
def read_json(path_to_df: Union[str, dict],include_concept_less:bool=False,consider_entity_tags:bool=True):
    _entities = []
    # Проверка какого типа переменная path_to_df
    if type(path_to_df) == str:
        # если строка - то это путь до файла с данными
        with open(path_to_df) as f:
            df = json.load(f)
    else:
        # В другом случае, считаем, что это сами данные - список документов
        df = path_to_df
    for s in df:
        sample_entities = []
        for _, ent in s['entities'].items():
            ...
            tag=None
            for tg in ent['tag']:
                if((tg=='ADR') | (tg=='Disease:DisTypeIndication')):
                    tag=tg
                    break
    ...
    return _entities #получаем массив по кол-ву документов, в таком виде[[('182', '192', 'Нарушение речи')], [('488', '504', 'Профилактика'), ('869', '885', 'Профилактика')]]
...
def compute_scores(gt_ent, pred_ent, log=None, consider_entity_tags=False):
    ...
    print("")
    gt_ner, pred_ner = _convert_by_setting(gt_ent, pred_ent, include_entity_types=False) #функция, которую можно включить с флажком false, для оценки кач-ва ner без нормализации
    gt_flat, pred_flat, types = _score(gt_ner, pred_ner) #создает 1мерный массивы, которые пойдут в f1 и список уникальных концептов которые есть в gt и pred
    ner_eval = _compute_metrics(gt_flat, pred_flat, types, print_results=True,save_results=False) #расчет f1 и распечатка результатов
    ...
    if (consider_entity_tags):
        ...
        label_ind=2
    gt_flat, pred_flat, types = _score(gt_ent, pred_ent,label_ind=label_ind) #создает 1мерный массивы, которые пойдут в f1 и список уникальных концептов которые есть в gt и pred
    ...
    norm_eval = _compute_metrics(gt_flat, pred_flat, types, print_results=False,save_results=True) #расчет f1 и распечатка результатов
