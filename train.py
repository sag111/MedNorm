...
parser.add_argument('-dict', '--meddra_path', type=str, 
                     help='Path to the MedDRA file in asc. format, for example pt.asc or llt.asc. It defines what dictionary you will use', default='./Demo/meddra_path/pt_rus.asc')
...
with open(train_data_path, 'r') as f:
    ...
    train_phrases, train_concepts = simple_parse_RDRS(train_data, CV, filter_conceptless=True)
    
if validation_data_path:
    with open(validation_data_path, 'r') as f:
        validation_data = json.load(f)
        val_phrases, val_concepts = simple_parse_RDRS(validation_data, CV, filter_conceptless=True)
        
if test_data_path:
    with open(test_data_path, 'r') as f:
        test_data = json.load(f)
        filter_conceptless = not args.use_conceptless
        test_phrases, test_concepts = simple_parse_RDRS(test_data, CV, filter_conceptless=filter_conceptless)
...
for epoch in range(1, train_config['epochs']+1):
    ...
    if validation_data_path:
        ...
        net.train()
        #print('---------\n')
        #print('The accuracies of the best model on the validation dataset')
        #print(valid_evaluator.f1_scores_df)
