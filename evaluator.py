...
class Evaluator:
    ...
    def get_f1_score(self, y_pred, average, idx='All'):
        if idx=='All':
            return f1_score(self.test_concepts, y_pred, average=average)
        elif idx=='In VOC':
            return f1_score(self.test_concepts[self.in_voc_idx], y_pred[self.in_voc_idx], average=average)
        elif idx=='Out of VOC':
            if len(self.test_concepts[self.out_of_voc_idx])!=0:
                return f1_score(self.test_concepts[self.out_of_voc_idx], y_pred[self.out_of_voc_idx], average=average)
            else:
                return 0
        
    def get_all_f1_scores(self, y_pred, pretty_print=True):
        ...
        for cfg_row in columns_tuples:
            all_f1_scores.append(self.get_f1_score(np.array(y_pred), 
