...
class SimpleMedNormDataset(torch.utils.data.Dataset):
    def __init__(self, X: List[str], y: List[str], transformer_model_path: str, 
                 concept_to_idx: dict, use_cuda = False):
        '''
        Входные данные: 
        X - список из фраз
    ...
    def _vectorization(self, phrases_text, concepts):
        ...
        for concept in concepts:
            concept_one_hot = copy(self.zero_meddra_vec)
            concept_one_hot[self.concept_to_idx[concept]]=1
            vectorized_concepts.append(concept_one_hot)
        ...
        vectorized_phrases_text = self.tokenizer(phrases_text, padding=True, truncation=True, return_tensors='pt')
