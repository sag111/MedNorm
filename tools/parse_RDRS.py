...
def simple_parse_RDRS(data, CV, filter_conceptless=True):
    entities_li = []
    codes_li = []
    for rev in data:
        for ent in rev['entities'].values():
            if 'MedDRA' in ent:
                code = CV.meddra_term_to_meddra_code[ent['MedDRA'].split('|')[0]] if ent['MedDRA'] else 'CONCEPT_LESS'
                if filter_conceptless and code=='CONCEPT_LESS':
                    continue
                entities_li.append(ent['text'])
                codes_li.append(code)
    return entities_li, codes_li

def simple_parse_sagnlp_RDRS(data):
    entities_li = []
    codes_li = []
    for rev in data:
        for ent in rev['entities'].values():
            if 'MedDRA' in ent and ent['MedDRA']:
                entities_li.append(ent['text'])
                codes_li.append(ent['MedDRA'])
    return entities_li, codes_li